    ...
    return this.checkUserExists(user.email).pipe(
      switchMap((exists) => {
        if (exists) {
          ...
          return this.http.post<User>(this.apiUrl, user); // Return the observable from the POST request
        }
      }),
      catchError((error) => {
    ...
    return this.http.get<User[]>(this.apiUrl, {
      ...
      map(users => {
        ...
        if (user.password === password) {
          ...
          return of(user); // Return an Observable with the user object if the password is correct
        } else {
          return of('Mot de passe incorrect.'); // Return an Observable with a message if password is incorrect
        }
      }),
      catchError((error) => {
        console.error('Erreur lors de la connexion', error);
        return of('Une erreur est survenue lors de la connexion.'); // Return an Observable with a generic error message
      }),
